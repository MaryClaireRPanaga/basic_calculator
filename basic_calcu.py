...
class ProCalculator(tk.Tk):
    ...
    def initialize_user_interface(self): # Descriptive Snake Case Method
        """Builds the UI using a grid system."""
        # Display Screen Area
        display_entry = tk.Entry(
            self, 
            textvariable=self.equation_text_variable, 
            font=('Arial', 32, 'bold'), 
            bg=self.color_screen_accent, 
            fg=self.color_button_main, 
            borderwidth=0, 
            justify="right"
        )
        ...
        for button_text in calculator_buttons:
            # Set button command based on identity
            if button_text == "=":
            ...
            # Creating the Salmon Pink buttons
            action_button = tk.Button(
                button_container_frame, 
                text=button_text, 
                font=('Arial', 14, 'bold'),
                bg=self.color_button_main, 
                fg="black",
                relief="flat",
    ...
    def append_to_expression(self, input_character):
        """Appends the button value to the current expression."""
    ...
    def calculate_final_result(self):
        """Calculates result with Exception Handling for runtime errors."""
        try:
            # Step 7: Use Python Function and Exception Handling
            ...
            # Step 4: Ask if the user wants to try again
    ...
    def ask_user_for_retry(self):
        ...
        if user_wants_to_continue:
            # Step 5: If yes, repeat Step 1
            self.clear_input_screen()
        else:
            # Step 6: If no, Display 'Thank you!' and exit
            messagebox.showinfo("Exit", "Thank you!")
            self.destroy()
if __name__ == "__main__":
    calculator_app_instance = ProCalculator()
    calculator_app_instance.mainloop()